...
console.log(textTag);
const pattern5 = /^\s*<img\s*src="(.+)"\s*\/>$/;
textTag.forEach((element, index) =>
    console.log("La dirección de la foto " + (index+1) + " es: " + element.match(pattern5)[1])
);

console.log("---- Validar sintaxis tarjeta de crédito ----");
console.log("---- Caso 1 ----");
const pattern6 = /^(5[0-5]\d{2})(\s|-)?(\d{4})(\s|-)?(\d{4})(\s|-)?(\d{4})$/;
const numberCard = ["5299 6400 0000 0000", "5299-6400-0000-0000" , "5299640000000000"];
console.log("---- Caso 2 ----");
numberCard.forEach(card => console.log("Para la tarjeta " + card + " los grupos de números son: Grupo 1 " + card.match(pattern6)[1]+ " Grupo 2 " + card.match(pattern6)[3] + " Grupo 3 " + card.match(pattern6)[5] + " Grupo 4 " + card.match(pattern6)[7]));
